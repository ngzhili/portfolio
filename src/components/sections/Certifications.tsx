/**
 * Certifications — an auto-scrolling marquee of credential cards.
 ...
 * card expands a detail panel below the strip with the full blurb + credential
 * link. Under prefers-reduced-motion the strip becomes a manual scroll-snap row
 * (see globals.css).
 */
'use client';
...
/** Initials fallback when a cert has no issuer logo (e.g. "DataCamp" → "DC"). */
function monogram(issuer: string): string {
  return issuer
    .replace(/[(].*?[)]/g, '') // drop parenthetical suffixes like "(CFI)"
    .split(/[\s,&–-]+/)
    .filter(Boolean)
    .slice(0, 2)
...
function CertCard({
  ...
  // (the logo PNGs may not be added yet).
  const [logoFailed, setLogoFailed] = useState(false);
  const showLogo = Boolean(cert.logo) && !logoFailed;
      ...
      <div className="flex items-center gap-3">
        ...
        >
          {showLogo ? (
            ...
            <span className="text-xs font-bold text-gradient">{monogram(cert.issuer)}</span>
          )}
        </span>
        <span className="min-w-0">
          ...
          <span className="block text-xs text-faint">{cert.date}</span>
        </span>
      </div>
...
export function Certifications() {
  // The selected cert (by title — stable & unique) whose detail panel is open.
  const [selected, setSelected] = useState<string | null>(null);
  // True while the pointer/touch is over the strip; pauses the marquee.
  const [hovering, setHovering] = useState(false);

  const active = certifications.find((c) => c.title === selected) ?? null;
  ...
  return (
    <Section
      ...
      intro="Specializations and courses I've completed."
    >
      <Reveal>
        <div
              ...
              >
                {certifications.map((cert) => (
                  <CertCard
                    key={`${copy}-${cert.title}`}
                    cert={cert}
                    active={active?.title === cert.title}
                    onToggle={() => handleCardClick(cert.title)}
                  />
                ))}
              </div>
            ))}
            ...
            <div className="rounded-2xl border border-accent/30 bg-surface/60 p-6">
              ...
              {active.credentialUrl && (
                <a
                  ...
                  className="mt-4 inline-flex items-center gap-1.5 text-sm font-medium text-text transition-colors hover:text-cyan"
                >
                  <ExternalLinkIcon /> Show credential
                </a>
              )}
            </div>
          )}
